        ...
        for (const user of unverifiedUsers) {
            if (USE_SUPABASE) {
              ...
              if (filesToDelete.length > 0) {
                  ...
                  if (error) console.error(`[Cleanup] Error deleting files for user ${user.id}:`, error);
                  else console.log(`[Cleanup] Deleted ${filesToDelete.length} files from storage.`);
              }
            }
            ...
            if (USE_SUPABASE) {
              ...
              if (filesToDelete.length > 0) {
                  const { error } = await supabase!.storage
                      .from(supabaseBucket)
                      .remove(filesToDelete);
                  if (error) console.error(`[Cleanup] Error deleting files for workspace ${workspace.id}:`, error);
                  else console.log(`[Cleanup] Deleted ${filesToDelete.length} files from storage.`);
              }
            }
