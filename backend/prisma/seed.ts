...
async function main() {
  ...
  await prisma.kanbanCard.deleteMany({ where: {} });
  await prisma.kanbanColumn.deleteMany({ where: {} });
  await prisma.kanbanBoard.deleteMany({ where: {} });
  await prisma.drawing.deleteMany({ where: {} });
  ...
  const passwordHash = await bcrypt.hash("devspark123", BCRYPT_ROUNDS);
...
- **Priya:** Roadmap doc updated with new Milestone 3 details`,
  ...
  const backendRoom = await prisma.chatRoom.create({
    data: {
      name: "backend-team",
      workspaceId: workspace.id,
    },
  ...
  const msg = async (roomId: string, authorId: string, content: string, offsetMinutes: number) => {
  ...
  const msg2 = async (roomId: string, authorId: string, content: string, offsetMinutes: number) => {
  ...
  const designFolder  = await prisma.folder.create({ data: { name: "design",  workspaceId: workspace.id } });
  const backendFolder = await prisma.folder.create({ data: { name: "backend", workspaceId: workspace.id } });
  const productFolder = await prisma.folder.create({ data: { name: "product", workspaceId: workspace.id } });
  const qaFolder      = await prisma.folder.create({ data: { name: "qa",      workspaceId: workspace.id } });
  ...
  // --- Sprint 12 ---
  const sprintBoard = await prisma.kanbanBoard.create({
    data: {
      title: "Sprint 12",
      workspaceId: workspace.id,
    },
  ...
  const sprintColumns = await Promise.all([
  ...
  // --- Bug Tracker ---
  const bugBoard = await prisma.kanbanBoard.create({
    data: {
      title: "Bug Tracker",
      workspaceId: workspace.id,
    },
  ...
  const bugColumns = await Promise.all([
