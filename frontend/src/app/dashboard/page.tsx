      ...
      <header className="p-6 bg-white dark:bg-slate-900 border-b dark:border-slate-800 shadow-sm sticky top-0 z-10 backdrop-blur-md bg-white/80 dark:bg-slate-900/80">
        <div className="max-w-7xl mx-auto flex justify-between items-center w-full">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              onClick={logout}
              className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl text-slate-500 hover:text-red-500 transition-all"
            >
              <LogOut size={20} />
            </button>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <h1 className="text-4xl lg:text-5xl font-black tracking-tight mb-2">
                Hello, {user?.username || user?.name || "Guest"} 👋
              </h1>
              <p className="text-slate-500 dark:text-slate-400 font-medium">
                Here are your active workspaces. Start collaborating today.
              </p>
            </div>
        ...
        <section className="space-y-6">
          <div className="flex items-center justify-between border-b dark:border-slate-800 pb-4">
            ...
            <div className="flex items-center gap-2 p-1 bg-slate-100 dark:bg-slate-900 rounded-lg">
              <button className="p-1.5 bg-white dark:bg-slate-800 shadow-sm rounded-md text-blue-600">
                <Grid size={18} />
              </button>
          ...
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-48 bg-slate-200 dark:bg-slate-900 rounded-3xl animate-pulse"
                />
              ))}
            </div>
          ) : (
              ...
              {workspaces.map((w) => (
                ...
                >
                  ...
                  <div className="relative space-y-6">
                    ...
                    <div>
                      <h2 className="text-2xl font-black group-hover:text-blue-600 transition-colors truncate">
                        {w.name}
                      </h2>
                        ...
                        {(w.members || []).slice(0, 3).map((m, i) => (
                          ...
                          >
                            ...
                            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover/avatar:opacity-100 flex items-center justify-center transition-opacity">
                              <span className="text-[6px] text-white font-bold truncate px-0.5">
                                {(
                                  m.user.username || m.user.email.split("@")[0]
                                ).slice(0, 5)}
                              </span>
              ...
              {workspaces.length === 0 && (
                <div className="col-span-full py-20 bg-white dark:bg-slate-900 border-2 border-dashed dark:border-slate-800 rounded-[40px] flex flex-col items-center justify-center gap-4 text-slate-400 text-center px-6">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-sm max-w-xs font-medium uppercase tracking-wider text-slate-400 dark:text-slate-600">
                      Create your first space using the bar above to start your
                      project.
                    </p>
                  </div>
                </div>
